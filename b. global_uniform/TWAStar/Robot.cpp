...
bool Robot::getRoute(int curr_slot, ResEntity ** ResEntity_list, TileType ** inMap, int inRow, int inCol, Point * buff, int numDest)
{
	SLSD_TWAStar a;
	this->currRoute = a.findPathTW(curr_slot, ResEntity_list, inMap, inRow, inCol,
		this->startPort.i, this->startPort.j, this->destPort.i, this->destPort.j, buff, numDest);
	//a.destroy();
	...
	{
		//开始预约路径上的资源;
		list <TWASPoint> ::iterator rit = this->currRoute.begin();
...
void Robot::printTW(ResEntity ** ResEntity_list, int & curr_slot)
{
	...
	list <TWASPoint>::iterator ir = this->currRoute.begin();
