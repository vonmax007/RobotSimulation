...
void VirtualEnvironment::CreateEnvSLSD()			//产生模拟一的场景
{
	...
	this->CreateDest_centralized_uniform();		//a. 类型
...
void VirtualEnvironment::StepRobot()
{
	...
	for (; it != this->robot_list.end(); ++it)
	{
		...
		{
			if (it->stay || it->isFinMission == 1 || it->isFinMission == 3)						  //重新刷新的时间到了，需要重新找路
			{
				//cout << endl << it->Rid << " 号机器人在时间槽 " << this->time_slot << " 刷新搜索中 ...\n**********************************\n";
				int tryCount = 0;
				while (tryCount++ < 5)
				{
					it->findKPath(this->real_map, (this->map_row - 1) * 3 + 1, (this->map_col - 1) * 3 + 1);
					it->selectBestRoute(this->ResEntity_list, this->time_slot);
					if (it->reserveRoute(this->ResEntity_list, this->time_slot))
					{
						//如果预约成功，跳出寻路;
						it->stay = false;
						break;
			...
			//else cout << endl << it->Rid << " 号机器人在时间槽 " << this->time_slot << " 前进一步 ...\n**********************************\n";
			//机器人前进
			//it->tltRef--;
			//判断是否在此停留?
			if (it->stay)continue;//stay跳出运动;
			else if (it->currRouteTime.begin()->leave_slot - it->currRouteTime.begin()->enter_slot > 0)
				it->currRouteTime.begin()->enter_slot++;//停留的话;
			else
			{
				++maRC;									   //累计活动机器人数量;
				it->currPos = *(++(it->currRoute.begin()));//机器人前进一步
				it->currRoute.pop_front();				   //删除已经前进的一步
				it->currRouteTime.pop_front();
														   //如果是到达目的地的话
			...
			//cout << it->Rid << " 号机器人完成了时间槽 "<< this->time_slot <<" 的操作 ...\n**********************************\n\n";
		}
	}
...
void VirtualEnvironment::CreateRobot()
{
	...
	for (int k = 0; k < this->numRobot; k++)
	{
		...
		*/
        //采用固定目的地的方法;
        //destport = this->numDestination - k % this->numDestination - 1;
		int destport = this->Destinations.front();//取得其目的地port ID
		this->Destinations.pop_front();
